...
const SOUNDS: Sound[] = [
  {
    ...
    label: 'Oceano',
    emoji: '🌊',
    description: 'Ondas lentas que acalmam o sistema nervoso',
    color: '#0891b2',
    bg: '#ecfeff',
  },
  {
    ...
    label: 'Chuva Suave',
    emoji: '🌧️',
    description: 'Gotas irregulares que distraem pensamentos acelerados',
    color: '#6366f1',
    bg: '#eef2ff',
  },
  {
    ...
    label: 'Floresta',
    emoji: '🌿',
    description: 'Ambiente natural com pássaros e folhas ao vento',
    color: '#16a34a',
    bg: '#f0fdf4',
  },
  {
    id: 'bowl',
    label: 'Tigela Tibetana',
    emoji: '🔔',
    description: 'Ressonância harmônica para meditação profunda',
    color: '#b388c4',
    bg: '#faf5ff',
  },
  {
    ...
    label: 'Ruído Rosa',
    emoji: '🎚️',
    description: 'Mascaramento suave de sons perturbadores',
    color: '#f59e0b',
    bg: '#fffbeb',
  },
];
...
function startOcean(ctx: AudioContext): (() => void) {
  ...
  [0.08, 0.12, 0.17].forEach((rate, i) => {
    ...
    src.loop = true;
    src.loopStart = 0;
    src.loopEnd = bufSize / ctx.sampleRate;
...
export default function CalmingSounds({ onBack, logActivity }: CalmingSoundsProps) {
  ...
  const play = (id: SoundId) => {
    ...
    // Insert master gain between generator and speakers using a proxy destination
    // Simpler: re-patch gain using a compressor as final
    const master = ctx.createGain();
    master.gain.value = volume;
    masterRef.current = master;

    // Temporarily override ctx.destination — not possible in Web Audio.
    // Instead, each generator connects to ctx.destination.
    // We'll control volume by scaling the gain inside each generator.
    // Practical solution: set a global gain multiplier when setting up.
        ...
        {activeSound && (
          ...
          >
            <div className="rounded-2xl px-4 py-3 flex items-center gap-3"
              style={{ background: activeSound.bg, border: `1.5px solid ${activeSound.color}44` }}>
              <div className="flex items-end gap-[3px] h-5">
                {[0.5, 1, 0.65, 1, 0.55].map((h, i) => (
                  <motion.div key={i} className="w-1 rounded-full"
                    style={{ background: activeSound.color, height: 20 }}
        ...
        {SOUNDS.map((sound, i) => {
          ...
          return (
            <motion.button key={sound.id}
              ...
              style={isActive
                ? { background: sound.bg, borderColor: sound.color, boxShadow: `0 4px 20px ${sound.color}2a` }
