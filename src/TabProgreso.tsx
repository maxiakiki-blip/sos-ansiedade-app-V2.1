...
export default function TabProgreso({ logs, moods, gamification, activityCounts }: TabProgresoProps) {
  ...
  const weekDays = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      dateStr,
      uniqueActs: (logs[dateStr] || []).length,
      totalSessions,
      mood: moods[dateStr],
  ...
  const weekUnique = weekDays.reduce((a, d) => a + d.uniqueActs, 0);

  // Most used exercise (all time raw)
      ...
      >
        {hasActivityToday ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black text-emerald-700">Missão do dia cumprida!</p>
              <p className="text-[11px] text-emerald-600 leading-snug">{dailyMsg}</p>
            </div>
          </div>
        ) : gamification.streak > 0 ? (
          <div className="flex items-center gap-3">
            <span className="text-2xl">🔥</span>
            <div>
              <p className="text-xs font-black text-orange-700">Não quebre sua racha de {gamification.streak} dias!</p>
              <p className="text-[11px] text-orange-600 leading-snug">Faça ao menos 1 exercício hoje para manter.</p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-3">
            <span className="text-2xl">💜</span>
            <div>
              <p className="text-xs font-black text-[#9d6eb5]">Missão de hoje</p>
              <p className="text-[11px] text-[#b388c4] leading-snug">{dailyMsg}</p>
            </div>
          </div>
        )}
      </motion.div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="w-full h-2 bg-gray-100 rounded-full mt-2 overflow-hidden">
              <motion.div className="h-full rounded-full"
                initial={{ width: 0 }}
                animate={{ width: `${Math.max(3, xpProgress)}%` }}
      ...
      <AnimatePresence mode="wait">
        {view === 'stats' && (
          <motion.div key="stats" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
            ...
            {/* Weekly bar chart — shows total sessions */}
            ...
            {/* Today's detail with session counts */}
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
                  ...
                  {todayActivities.map((act, i) => {
                    ...
                    return (
                      <motion.li key={i}
                        ...
                        {count > 1 && (
                          <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-[#b388c4]/10 text-[#b388c4]">
                            ×{count}
                          </span>
                        )}
                    ...
                    {catBadges.map(badge => {
                      ...
                      return (
                        <motion.div key={badge.id}
                          ...
                          style={{
                            background: isUnlocked ? 'linear-gradient(135deg, #F5EFFF, white)' : '#f8fafc',
                            borderColor: isUnlocked ? 'rgba(179,136,196,0.35)' : '#e5e7eb',
                          }}>
                          <div className="flex items-start justify-between mb-1">
                            <span className="text-2xl" style={{ filter: isUnlocked ? 'none' : 'grayscale(100%) opacity(0.4)' }}>
                              {badge.emoji}
                            </span>
                            {isUnlocked
                          ...
                          {badge.xpReward > 0 && isUnlocked && (
                            <span className="text-[9px] font-black text-[#b388c4]">+{badge.xpReward} XP ganho</span>
                          )}
                          {badge.xpReward > 0 && !isUnlocked && (
                            <span className="text-[9px] font-bold text-gray-300">+{badge.xpReward} XP ao desbloquear</span>
                          )}
                        </motion.div>
                      );
                    })}
                      ...
                      <div className="w-8 h-8 rounded-full flex items-center justify-center relative"
                        style={{
                          background: day.sessions > 0
                            ? `rgba(179,136,196,${0.3 + intensity * 0.7})`
                            : '#f8fafc',
